...
def Psi2_fromWaveform(t_vec, r, p_r, phi, p_phi, p):
    dt = t_vec[1]-t_vec[0]
    sigma_sigma_Dot =  (abs(ODEs.Rh22(r, p_r, phi, p_phi, p))**2)*ODEs.dphi_by_dt(r, p_r, phi, p_phi, p)
    abs_Sigma_Dot_sqr = (ODEs.dphi_by_dt(r, p_r, phi, p_phi, p)*abs(ODEs.Rh22(r, p_r, phi, p_phi, p)))**2
    Int_abs_Sigma_Dot_sqr = np.cumsum(abs_Sigma_Dot_sqr)*dt
    Psi2_diff = sigma_sigma_Dot - Int_abs_Sigma_Dot_sqr -  abs(ODEs.Rh22(r, p_r, phi, p_phi, p))
...
plt.plot(t_vec, Psi2_fromWaveform(t_vec, r, p_r, phi, p_phi, p))
plt.xlim(0, 5900)
plt.ylim(-0.3, 0.1)
plt.xlabel(r'$time$')
...
print(ODEs.Rh22(r, p_r, phi, p_phi, p).real)
