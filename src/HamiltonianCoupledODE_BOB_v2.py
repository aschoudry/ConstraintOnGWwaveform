...
def Coupled_HamiltonianODEs_solver(initial_condition, domain, parameter_values):
    ...
    nu, t0, tp, tau, Ap, r_switch, rad_reac, omega0 = parameter_values
    t_vec = domain
	
    abserr = 1.0e-8                         # error control parameters
    relerr = 1.0e-6

    yvec = odeint(derivs, w0, t_vec, args = (nu, t0, tp, tau, Ap, r_switch, rad_reac, omega0), atol=abserr, rtol=relerr)
    return yvec


'Derivatives'

def derivs(w, t_vec, nu, t0, tp, tau, Ap, r_switch, rad_reac, omega0):

    r, p_r, phi, p_phi = w

    dphi_bydt= dphi_by_dt(r, p_r, phi, p_phi, nu)
    dr_bydt= dr_by_dt(r, p_r, phi, p_phi, nu)
    dp_phi_bydt= dp_phi_by_dt(r, p_r, phi, p_phi, nu, t_vec, t0, tp, tau, Ap, r_switch, rad_reac, omega0)
...
def dp_phi_by_dt(r, p_r, phi, p_phi, nu, t_vec, t0, tp, tau, Ap, r_switch, rad_reac, omega0):
    ...
    if r >= r_switch:
        ...
        if rad_reac==1:
            forceRR = f_phi_BOB(omega0, tau, t_vec, t0, tp, Ap)
        else:
            forceRR = 0.0
...
def f_phi_BOB(omega0, tau, t_vec, t0, tp, Ap):  
