...
plt.plot(t[t_ref_idx-idx_pre:], Waveform_BOB(t, tp, Ap, OMEGA_ref, OMEGA_dot_ref, OM_QNM, tau, phase0)[t_ref_idx-idx_pre:])
plt.plot(t, hp, 'r--')
plt.xlim(-60, 80)
plt.show()
...
plt.plot(t, amp_bob*np.cos(2*phase_BOB))
plt.plot(t+2.5, hp, 'r--')
plt.xlim(-60, 80)
plt.show()
...
#plt.plot(x, 2*OM_BOB + 0.5*np.sqrt(OM_BOB), 'b--')
