...
# 22 mode for spin weighted spherical harmonic
def m2Y22(theta, phi):
    return (1.0/8.0)*np.sqrt(5.0/np.pi)*(1+np.cos(theta))**2*np.exp(2*1j*phi)
...
# Compute Psi2 from NR data
mts_h = sxs.waveforms.memory.MTS(w_sliced_L3)
eth2_h = mts_h.eth.eth*m2Y22(0.0,0.0).real
h_dot_h_bar = (mts_h.dot*mts_h.bar)*abs(m2Y22(0.0,0.0))**2
...
t_shift_idx = BOB.find_nearest1(psi2_22, max(psi2_22))
time=time-time[t_shift_idx]


plt.plot(time, 4.0*psi2_22)
plt.plot(time_bob, h22_bob)

plt.show()

plt.plot(time, h_dot_h_bar[:,h_dot_h_bar.index(2, 0)].data.real, '--')
plt.plot(time, eth2_h[:,eth2_h.index(2, 0)].data.real)
plt.show()
